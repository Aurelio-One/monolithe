...
import { Controller, Parallax, Autoplay } from 'swiper'
...
function SliderPortfolio({
  ...
  const swiperChange = (swiper) => {
    ...
    const [newTitle, oldTitle] = [
      newContent('.title-heading .char'),
      oldContent('.title-heading .char'),
    ]
    ...
    tl.fromTo($isRight ? oldTitle : oldTitle.reverse(), 1, animate.show, {
      ...animate.hide,
      onComplete: () => setActive(Number.parseInt(newNum, 10)),
    })
  ...
  return (
    <Tag
      className={dsnCN(
        'root-slider half-content v-dark-head background-main',
        className
      )}
      ref={rootSlider}
    >
      <div
        className={`bg-image hero-img ${typeBg}`}
        ref={bg}
      >
        <Swiper
          {...optionSwiper}
